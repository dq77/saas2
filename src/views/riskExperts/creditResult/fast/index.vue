...
.page-main
  ...
  .p-control
    el-button(type="primary", @click="toAccurate" v-weblogs.click='"赊销评估-快速评估-评估结果页-一键获取"' :loading='loading') 一键获取精准评估结果
    ...
    state.detailState = useAsync({
      ...
      successCallBack: ({ code, data }) => {
        if (code === '0') {
          ...
          const { operatingConditions, enterpriseSize, enterpriseDevelopment, comprehensiveStrength, enterpriseCompetitiveness } = (data.fastRiskProfile || {})
          state.dataSource = [operatingConditions, enterpriseSize, enterpriseDevelopment, comprehensiveStrength, enterpriseCompetitiveness]
        }
      }
    })
    // 创建精准流程
