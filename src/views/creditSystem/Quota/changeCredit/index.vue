...
export default {
  ...
  setup(props, { emit }) {
    // ajax
    const { credit: { changeCreditApply } } = ajaxStore
    // ref
    const formRef = ref()
    // form
    ...
    const successCallBack = ({ code }) => {
      if (code === '0') {
        Message.success('变更授信成功')
        cancleHandle()
        emit('successHandle')
      }
    }
    // 变更授信接口
    const { doResult, loading } = useAsync({ request: changeCreditApply, init: false, successCallBack })
    // 取消
    const cancleHandle = () => {
    ...
    const sureHandle = () => {
      ...
      submitHandle(() => {
        const { newValidTo, newCreditLine, reason } = formData
        const { detail: { creditAccountId } } = props
        doResult({ newValidTo, newCreditLine, reason, creditAccountId })
      })
    }
    // state
