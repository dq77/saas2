...
.loanDetail
  ...
  div(style="padding: 20px;")
    Table(:dataSource="dataSource", :columns="collectionColumns", :loading="loading")
...
import { collectionColumns } from '../config'
...
export default {
  ...
  setup() {
    const formRef = ref()
    const { credit: { debtDetailList } } = ajaxStore
    // form
    const {
      formData,
      formConfig,
    ...
    } = useTable({ request: debtDetailList })
    // 查询
