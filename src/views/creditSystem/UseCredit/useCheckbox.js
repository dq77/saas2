...
export default ({
  ...
  const validate = (row = null) => {
    ...
    if (row) {
      // 单独校验
      const { id, orderCloseAmount } = row
      const isError = (!orderCloseAmount && orderCloseAmount !== 0)
      ...
      // 批量校验
      const filterDatas = selectDatas.value.filter(item => (!item.orderCloseAmount && item.orderCloseAmount !== 0)) || []
  ...
  const selectHandle = ({ row }) => {
    row.orderCloseAmount = undefined
    row.isError = false
  }
  // 全选置空数据并重置校验
  const selectAll = () => {
    ...
    dataSource && dataSource.forEach(item => {
      // 取消校验&置空关单金额
      item.orderCloseAmount = undefined
      item.isError = false
    })
  }
  // checkbox勾选数据发生变化时
