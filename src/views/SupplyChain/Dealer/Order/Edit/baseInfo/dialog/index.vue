...
export default {
  ...
  setup(props, { emit }) {
    ...
    const selectChange = async ({value, key}) => {
      if (key === 'projectId' && value) { // 项目
        ...
        if (provinceCode && province && cityCode && city && regionCode && region && detailAddress) {
          ...
          formState.setFormItem('addressNameText', addressName.join(' / '))
      ...
      } else if (key === 'addressCode' && value) { // 地址
        formState.setFormItem('addressName', getRegionTextArr(...value))
    ...
    const initCacheBaseInfo = async (newFormData) => {
      // 表单有数据则执行inCache
      if (newFormData?.paymentMethod?.id) {
