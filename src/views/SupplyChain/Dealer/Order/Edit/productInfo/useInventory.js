...
export default function () {
  ...
  // 查询库存配置
  useAsync({
    request: ajaxStore.dealer.getInventoryConfig,
    successCallBack: (res) => {
  ...
  // 下单失败，后端返回超库存时设置超库存产品数据
  const excessHandle = (resData) => {
    // 如果超库存
    if (resData.orderLowStock) {
      state.excessInventory = resData.orderItemDTOList.map(item => item.underStock === true)
      return true
    }
    return false
  }
  ...
  // 重新编辑时，清楚库存报错
  const clearExcessDataByIndex = (index) => {
    state.excessInventory[index] = false
  }
  ...
  const setCurrentInventorys = ({ rows, orderId }) => {
    batchQueryInventory({
      rows,
      orderId,
      request: ajaxStore.dealer.getCurrentInventorys
    })
  }
