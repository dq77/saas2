...
.page-main
  ...
  Card.p-mg(noTitle)
    .p-table-header-btns
      el-button(type='primary' @click='clickCheckIn') 登记入账
    //- table
    Table(
...
export default {
  ...
  setup(props, { root }) {
    ...
    const clickDetail = (row) => root.$router.push({
      name: 'supplyChain_factory_finance_enterAccountDetail',
      query: {
        billNo: row.billNo,
        customerName,
        customerId
    ...
    // 点击登记入账
    const clickCheckIn = () => root.$router.push({
      name: 'supplyChain_factory_finance_enterAccount',
      query: {
        customerName,
        customerId
      }
