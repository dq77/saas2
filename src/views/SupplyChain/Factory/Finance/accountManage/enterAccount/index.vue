...
.page-main
  Card(noTitle)
    el-form(
      ...
      el-row(:gutter='20')
        el-col(:span='20')
          el-form-item(label='入账账户与金额（元）：' prop='inBillDetails' :required='true')
            template(v-for='(row, index) in formData.inBillDetails')
              el-row.p-account-list-row(
                ...
                el-col(:span='4' v-if='!isDetailPage')
                  el-button(
                    icon='el-icon-delete'
                    v-if='formData.inBillDetails.length>1'
      ...
      template(v-if='formData.settlementType === "1"')
        ...
        el-row(:gutter='20')
          ...
          el-col(:span='10')
            el-form-item(label='实际入账时间：' prop='inOutTime' :required='true')
      ...
      template(v-if='formData.settlementType === "2"')
        el-row(:gutter='20')
          el-col(:span='10')
            el-form-item(label='实际入账时间：' prop='inOutTime')
              el-date-picker.width-100(
                type='datetime'
                format='yyyy-MM-dd HH:mm'
                :clearable='false'
      ...
      el-row(:gutter='20')
        el-col(:span='10')
          el-form-item(label='入账凭证：' prop='fileAddRequestList')
            Upload(
              v-if='!isDetailPage'
              ref='uploadRef'
              :action='uploadUrl'
...
export default defineComponent({
  ...
  setup(props, { root }) {
    ...
    const isDetailPage = root.$route.name === 'supplyChain_factory_finance_enterAccountDetail'
    ...
    formRules.inBillDetails[0].validator = (rule, value, callback) => {
