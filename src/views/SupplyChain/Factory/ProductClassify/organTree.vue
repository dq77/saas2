...
div
  .org-btn
    el-button(type="primary" @click="addFirst") 添加一级分类
        ...
        span.p-tree-node(slot-scope="{ node, data }")
          ...
          .p-edit
            el-tooltip(placement="top" content='删除' v-if='!data.subCategorys || (data.subCategorys && !data.subCategorys.length)')
              el-button.p-link-color(type="text" size="mini" @click.stop="() => remove(node, data)")
                i.p-icon.el-icon-remove-outline
            template()
              el-tooltip(placement="top" content='修改')
                el-button.p-link-color(type="text" size="mini" @click.stop="() => edit(node, data)")
                  i.p-icon.el-icon-edit-outline
              el-tooltip(placement="top" content='添加' v-if="data.level<5")
                el-button.p-link-color(type="text" size="mini" @click.stop="() => append(node,data)")
                  i.p-icon.el-icon-circle-plus-outline
              el-tooltip(placement="top" content='置顶')
                el-button.p-link-color(type="text" size="mini" @click.stop="() => handleTop(node,data)")
  ...
  el-dialog(
      ...
      .p-dialog-footer(slot="footer")
        el-button(@click='dialogNode.visible = false') 取 消
