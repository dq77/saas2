...
export const collapseItems = [
  { key: 'projectInfo', label: '项目信息' },
  { key: 'priceInfo', label: '产品和价格信息' },
]

export const defaultActiveCollapse = ['projectInfo', 'priceInfo']
...
export const getTableColumns = (routeType) => {
  if (routeType === 'detail') {
    return [
      ...
      { label: '项目单价（元）', key: 'price', type: 'input', minWidth: 120, disabled: true },
    ]
  } else {
    return [
      { label: '图片', key: 'mainPicPath', minWidth: 80 },
      ...
      { label: '产品规格', key: 'specification', minWidth: 100 },
      { label: '基础单价（元）', key: 'basePrice', minWidth: 120 },
      { label: '项目单价（元）', key: 'price', type: 'inputNumber', minWidth: 120, min: 0, precision: 4 },
    ]
  }
}
...
export const getRouteConfig = (routeName) => {
  const status = {
    projectPriceCreate: 'create',
    projectPriceEdit: 'edit',
    projectPriceDetail: 'detail'
  }
