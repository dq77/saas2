// 本页面用于项目价格编辑、查看，实际还包含创建功能，但因项目价格列表页为全量项目，所以不适合有创建功能，未来需要创建页可直接使用，只要添加路由到此，路由名称为projectPriceCreate即可。
  ...
  .page-layout-approval-content
    ...
    .p-submit-wrap(v-if='showSubmitBtns')
      el-button(
        ...
        weblogs-anchor="supplyChain-factory-price-project-edit-submit"
      ) 提交
      el-button(@click="submitState.backHandle" weblogs-anchor="supplyChain-factory-price-project-edit-return") 返回
</template>
...
export default {
  ...
  setup(props, { root }) {
    ...
    const { projectId, projectName } = root.$route.query
    ...
    // 表单 变更config中的项目名称下拉项
    const getProjectNameOptions = async (val) => {
      const res = await ajaxStore.factory.getProjectList({ name: val })
      if (res.data && res.data.code === '0') {
        formState.setConfig('projectId', 'options', res.data.data.pagedRecords.map(item => {
    ...
    const formState = useForm({
      ...
      formConfig: getFormConfig(routeType, {
        getOptions: getProjectNameOptions,
        defaultOptions: [
    ...
    if (['edit', 'detail'].indexOf(routeType) > -1) {
      formState.setFormItem('projectId', projectId)
    }
