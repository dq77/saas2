...
.page-main(v-loading='detailState.loading')
  ...
  el-collapse(v-model='activedCollapses')
    el-collapse-item(v-for='(item, index) in detailState.bagList' :name='item.id' :key='item.id')
      ...
      .u-head
        ...
        .u-head-right
          //- el-button(@click='() => signClick(item)' v-if='item.receiptStatus === 0' type='primary' round) 签收
...
export default {
  ...
  setup(props, { root }) {
    ...
    const activedCollapses = computed(() => detailState.bagList.map(item => item.id))
