...
export default function ({
  ...
  // 获取经销商options的封装
  const getCustomerOptions = async (query) => {
    formState.setConfig('customerId', 'loading', true)
    const res = await ajaxStore.factory.getCustomerList({ customerName: query })
    ...
    if (res?.data?.code === '0') {
      ...
      formState.setConfig('customerId', 'options', options)
    }
  }

  // 项目 remoteMethod
  const getProjectIdOptions = async (customerId) => {
    const res = await ajaxStore.factory.getProjectListByCustomer({ customerId })
  ...
  const getReceiptPersonOptions = async () => {
    const res = await ajaxStore.common.order.getCommonlyUsedList({ type: 'common_person_phone', isTrusted: true })
    if (res?.data?.code === '0') {
  ...
  const getPaymentMethodOptions = async () => {
    const res = await ajaxStore.factory.getPayTypeList({})
    if (res?.data?.code === '0') {
  ...
  const getInvoiceOptions = async (query) => {
    ...
    const res = await ajaxStore.factory.getInvoiceOptions({
  ...
  const getFreightPayId = async () => {
    const res = await ajaxStore.factory.getPayTypeList({dictCategoryCode: 'freightPayType'})
    if (res?.data?.code === '0') {
      const options = res.data.data[0].childrenList.map(item => {
        return {
          label: item.dictName,
