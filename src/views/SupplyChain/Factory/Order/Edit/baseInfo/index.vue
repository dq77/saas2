...
.p-base-info
  ...
  .u-detail-mode(v-else-if='formResult')
    .u-detail-col
      .u-row
        span.u-label 客户：
        span.u-value {{ formResult.customerId.customerName }}
      .u-row(v-if='formResult.purchaseType === "1"')
        span.u-label 项目：
      ...
      .u-row
        span.u-label 收货地址：
        span.u-value {{ formResult.addressName.join('，') }}
          span(v-if='formResult.address') ，{{ formResult.address }}
      .u-row
        span.u-label 开票信息：
...
export default {
  ...
  setup(props, { emit }) {
    ...
    // 用于父组件重置静态数据，适用于编辑页
    const initEditBaseInfo = (newData) => {
      formResult.value = newData
      mode.value = 'edit'
      dialogRef.value.initEditData(newData)
      emit('complete', formResult.value)
    }
