...
.p-product-info
  ...
  .p-table
    Table(
      ...
      :rowClassName='tableEditState.rowClassName'
      ...
      template(slot='goodsPrice' slot-scope='{row}')
        ...
        div(v-else)
          el-input-number(
            ...
            @change='v => tableEditState.cellChangeHandle(v, "goodsPrice", row)'
          )
        ...
        template(v-if='row.addedCostPriceVOS && row.addedCostPriceVOS.length')
          div(v-for='(addedPrice, index) in row.addedCostPriceVOS' :key="addedPrice.code")
            ...
            el-input-number(
              ...
              @change='v => tableEditState.cellChangeHandle(v, "addedCostPriceVOS", row)'
            )
        span(v-else) - -
      template(slot='quantity' slot-scope='{row}')
        div(style="display: flex;")
          el-input-number(
            ...
            @change='v => tableEditState.cellChangeHandle(v, "quantity", row)'
          )
          DiscountForQuantity(
            v-if='row.marketingLogos'
      ...
      template(slot='payAmount' slot-scope='{row}')
        ...
        span(v-else) - -
    .p-total-amount(v-loading='tableEditState.batchCaclulateLoading')
      span.u-count 总计：共{{tableState.dataSource.length}}件产品
      span.u-amount
        div 订单总金额：￥ {{totalAmount}}
</template>
...
export default {
  ...
  setup(props, { root }) {
    ...
    const totalAmount = computed(() => {
      ...
      return toAmountStr(num, 4, true)
    })

    // 是否已填写基础信息，用于判断表格行是否可删减等
    const hasBaseInfo = computed(() => {
      return tableEditState.customerId
    })
