...
export default {
  ...
  setup(props) {
    ...
    const sortRef = ref()
    // ajax
    const { receive: { getReceivablePaymentDataView } } = ajaxStore
    // form
    ...
    // table
    const tableApis = useTable({ request: getReceivablePaymentDataView, defaultParams: { customerId: props.customerId } })
    // table-查询
    const search = (params) => {
