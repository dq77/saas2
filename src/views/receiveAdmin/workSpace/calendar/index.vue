<template lang="pug">
.calendar_box
  Header(title='应收日历' :left='20' style='width: 100%;')
...
import { getDataByTime } from '../../utils'
...
export default {
  ...
  setup() {
    // ajaxStore
    const { receive: { calendarDetailList } } = ajaxStore
    // today
    ...
    // title
    const title = computed(() => state.cacheDate === date ? '今日应收提醒' : state.cacheDate)
    // cancleHandle
    const cancleHandle = () => {
      state.date = ''
      state.visible = false
