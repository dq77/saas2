...
div
  ...
  Card.p-mg(noTitle)
    ...
    Table(
      ...
      template(slot='accountTypeNameTh' slot-scope='{column}')
        span {{ column.label }}
        el-tooltip
          div(
            slot='content'
            style='width: 200px;'
          ) 当产品标签选择了优先扣款账户后，在进行下单支付时，会优先（或指定）通过下单产品标签所关联账户的进行扣款。
          i.el-icon-info
...
export default {
  ...
  setup() {
    ...
    const tableState = useTable({
      request: ajaxStore.factory.finance.getProductSignList,
    })
