...
export default defineComponent({
  ...
  setup(props, root) {
    ...
    // 创建
    const { doResult: createProductSign, loading: createLoading } = useAsync({
      request: ajaxStore.factory.finance.createProductSign,
      init: false
    })
    // 更新
    const { doResult: updateProductSign, loading: updateLoading } = useAsync({
      request: ajaxStore.factory.finance.updateProductSign,
      init: false
    })
    // 账户下拉
    const { doResult: getAccountTypeAllList } = useAsync({
    ...
    const clickSure = async () => {
      ...
      if (vRes) {
        ...
        if (props.mode === 'create') { // 新建时
          const res = await createProductSign(data)
          if (res?.code === '0') {
            Message.success('新增产品标签成功')
            root.emit('success')
          }
        } else { // 编辑时
          data.id = props.editData.id
          const res = await updateProductSign(data)
          if (res?.code === '0') {
            Message.success('编辑产品标签成功')
            root.emit('success')
          }
        }
      }
    ...
    const initBeforeShowWhenCreate = async () => {
      title.value = '新建产品标签'
      const res = await getAccountTypeAllList({ status: 1 })
      if (res?.code === '0') {
        formState.setConfig('accountTypeCode', 'options', res?.data || [])
      }
    }
