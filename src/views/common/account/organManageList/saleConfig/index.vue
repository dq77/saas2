    ...
    Card.list-page_table-wrapper(noTitle noWrap)
      .list-page_head-buttons
        el-popover(
          ...
          p 请添加销售组织；如果没有销售组织可配，请默认添加一条以公司名称命名的销售组织记录
          el-button(type="text" style="float: right;" @click="visible = false") 确定
          el-button(type="primary" slot='reference' @click="addHanlde") 添加销售组织
      .list-page_table
...
export default {
  ...
  methods: {
    ...
    _getList(params) {
      this.loading = true
      const { getSaleList } = ajaxStore.common.account
      getSaleList(params).then((res = {}) => {
        const { data: { code, data } } = res
        const { pageNo, totalCount, pagedRecords } = data || {}
        if (code === '0') {
          this.visible = pagedRecords ? !pagedRecords.length : false
