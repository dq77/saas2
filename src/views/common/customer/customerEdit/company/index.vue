...
Card(noTitle v-loading='detailState && detailState.loading')
  Form(
    ...
    template(slot='customerName' slot-scope='{ data, config }')
      select-company-name-by-qcc(
        :companyName='data.customerName'
        :disabled='config.disabled'
        @change='formEditState.selectCompanyName'
      )
    template(slot='loginFlagLabel')
      span 客户登录账号开通
      el-tooltip(placement='top')
        div(slot='content')
          | 输入联系手机号后可开通客户登录账号<br>账号开通后无法再修改企业名称、社会信用代码、当前客户登录账号
        i.el-icon-info
  .p-btns
    el-button(
      type='primary'
...
import SelectCompanyNameByQcc from '@/components/SelectCompanyNameByQcc'
