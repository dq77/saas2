<template lang="pug">
Card.p-card(title='销售面积/拿地面积趋势' v-loading='loading')
  #saleAreaChart(v-if='dataSource.length')
...
import { doubleLineOption, saleAreaLegend, saleAreaYData } from './config'
...
export default {
  ...
  setup(props) {
    ...
    const {
      ...
      option: setNewOption({newLegend: saleAreaLegend, newYData: saleAreaYData}),
      id: 'saleAreaChart',
      isInit: false
    })
    // 设置option
    const _setDataSource = (data) => {
      ...
      for (const key in saleData) {
        xData.push(key)
        sellArea.push(saleData[key].sellArea)
        acquireArea.push(saleData[key].acquireArea)
      }
      setxData(xData)
      setDataSource(sellArea, 0)
      setDataSource(acquireArea, 1)
    }
