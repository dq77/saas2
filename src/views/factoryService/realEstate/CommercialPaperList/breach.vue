...
Card(noTitle)
  Prompt
  Table(:columns='breachColumns' :dataSource='dataSource' :loading='loading')
...
import { breachColumns } from './config'
...
export default {
  ...
  setup(props) {
    const { getBreachList } = ajaxStore.factoryService.realEstate
    const {
      loading,
      dataSource,
      pagination,
