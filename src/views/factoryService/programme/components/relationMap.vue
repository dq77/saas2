  ...
  methods: {
    ...
    */
    async getData(entityType, entityName, level, companyName) {
      ...
      this.loading = true
      const res = await ajaxStore.factoryService.programme.getRelationMap(params)
      this.loading = false
    ...
    async getPersonNodeInfo(personName, name) {
      const res = await ajaxStore.factoryService.realEstate.getPersonNodeInfo({
        name,
        creditCode: this.regCode,
        personName,
        companyName: this.rootCompanyName
