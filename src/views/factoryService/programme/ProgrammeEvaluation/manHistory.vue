...
.page-main
  ...
  list-page(
    ...
    template(slot='operation' slot-scope='{row}')
      ...
      el-button(type='text' v-if='row.status === "1"' @click='cancelItem(row.id, row.processId)' weblogs-anchor='factory-customerEvaluation-eval-history-cancel') 取消
      el-button(type='text' v-if='row.status === "0"' @click='deleteItem(row.id, "1")') 删除
      el-button(type='text' @click='checkDetail(row.id)' weblogs-anchor='factory-customerEvaluation-eval-history-view') 查看
</template>
<script>
...
export default {
  ...
  data() {
    return {
      tabItem: 'programmeEvaluationMan'
    }
  },
  computed: {
    tabs() {
    ...
    listPageConfig () {
      const subAccount = { name: '发起人工评估的账号', property: 'subAccount', minWidth: '130' }
      return {
        searchParams: [
          [
            {
        ...
        tableHeader: [
          ...
          this.tabItem === 'programmeEvaluationMan' && subAccount,
