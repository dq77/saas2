...
export default {
    ...
    listPageConfig() {
      return {
        searchParams: [
          [
            ...
            },
            {
              type: 'daterange',
              label: '列入日期',
              formItemName_start: 'inStartDate',
              formItemName_end: 'inEndDate',
        ...
        tableHeader: [
          ...
          { name: '列入日期', property: 'inDate', minWidth: '85', sortable: 'custom', align: 'right' },
          { name: '列入经营异常名录原因', property: 'inCausion', minWidth: '140' },
          { name: '决定机关名称', property: 'inDecisionOrg', minWidth: '100'},
          { name: '移出日期', property: 'outDate', minWidth: '85', sortable: 'custom', align: 'right', formatter: v => v || '-' },
          { name: '移出经营异常名录原因', property: 'outCausion', minWidth: '140', formatter: v => v || '-' },
        ],
        getTableList: {
          type: 'component',
          fn: ajaxStore.factoryService.riskEngine.getAbnormalOperationList
        },
  ...
  methods: {
    ...
    initSearchParams(params) {
      const { inStartDate, inEndDate, province, city, descs, ascs } = this.$route.query
    ...
    toDetail(caseId) {
      ...
      window.open(`https://${process.env.VUE_APP_WEBTOOLS_HOST}/sentiment/abnormalOperation/detail?caseId=${caseId}`)
    },
    async createBlocksData() {
      const res = await this.$store.dispatch('getConsensusStatistics')
      this.blocksData = [
        { label: '案件总数（个）', number: res.jyycTotalCount, iconName: '17.png' },
        { label: '经营异常经销商数（个）', number: res.jyycDealerCount, iconName: '11.png' },
      ]
    }
  },
  mounted() {
