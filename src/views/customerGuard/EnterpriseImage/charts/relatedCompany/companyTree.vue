...
export default {
  ...
  watch: {
    ...
    'upData' (data) {
      if (data) {
        ...
        this.rootName = [this.companyName]
        this.treeInit()
      }
    },
    'downData' (data) {
  ...
  methods: {
    ...
    async getChildren(companyName, direction) {
      const { getRelatedCompanyByLevel } = ajaxStore.customer.enterprise
    ...
    treeInit() {
      ...
      const centralX = canvasHeight / 2 + margin.top
