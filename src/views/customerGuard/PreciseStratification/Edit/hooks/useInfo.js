...
export default ({params}) => {
  const { customer: { precise: { getCompanyBasic, getPersonBasicList, getCompanyStockholder, getMaterialList, getHouseList } } } = ajaxStore
  ...
  const getPerson = (data, isInit) => {
    data.forEach(person => {
      ...
      isInit && state.houseState.doResult({
        owner: person.code
      }).then(({ code, data }) => {
  ...
  const init = () => {
    ...
    state.personState.doResult(params).then(({ code, params, data }) => {
      code === '0' && getPerson(data, true)
    })
  }
  init()

  // 重新拉取资料附件
  const reloadMaterialList = () => {
    state.materialState.doResult({
      owner: params.companyBasicCode,
      ownerType: '2',
    }).then(({code, data}) => {
