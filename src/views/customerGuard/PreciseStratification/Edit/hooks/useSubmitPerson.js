...
export default ({
  ...
  const checkTable = () => {
    return !getTableState().dataSource.some(item => {
      if (!item.rightHolder) Message.error('请填写实控人房屋所有人')
      // if (!item.locateLocationCode) Message.error('请填写实控人房屋省市区')
      // if (!item.locateLocation) Message.error('请填写实控人房屋详细地址')
      // if (!item.houseFloor) Message.error('请填写实控人房屋楼层')
      // if (!item.buildingArea) Message.error('请填写实控人建筑面积')
      // if (!item.houseValue) Message.error('请填写实控人房产价值')
