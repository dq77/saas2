...
div(v-loading='loading')
  List(
    title='指标'
    :dataSource='income.tableList || []'
    :columns="income.tableHeader || []"
  )
</template>
...
import { publicTableHeader, incomeTableList } from './config'
...
export default {
  ...
  setup (props, { root }) {
    ...
    const { customer: { stratify: { getIncomeByQuarter } } } = ajaxStore
    const { loading } = useAsync({
      ...
      successCallBack: res => {
        ...
        state.income = getTableData(data, incomeTableList)
      }
    })
